/*
 * S3 IMU+Encoder — LEG 1 | WROOM: 08:3A:F2:52:88:E8
...
bool initIMU(TwoWire &w,int sda,int scl,const char* label){
  ...
  if(err!=0){Serial.printf("[%s] Scanning...\n",label);for(uint8_t a=1;a<127;a++){w.beginTransmission(a);if(w.endTransmission()==0)Serial.printf("[%s] Found 0x%02X\n",label,a);}return false;}
...
Serial.println("=== LEG 1 IMU+Enc ===");pinMode(LED_PIN,OUTPUT);
