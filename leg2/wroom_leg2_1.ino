/*
 * WROOM hub — LEG 2 | Receives ESP-NOW from leg2_imu + leg2_emg, optional cal commands to IMU.
 * Board: ESP32 Dev Module (or ESP32-S3) | Channel 1 must match leg sketches.
 *
 * 1) Flash this on the WROOM whose STA MAC is 08:3A:F2:52:88:E8 (see leg2_* wroomMAC[]).
 * 2) Run ESP_MAC_FINDER on the Leg 2 IMU S3 and Leg 2 EMG S3; paste bytes below.
...
// Leg 2 sender boards (STA MAC from each ESP32-S3)
...
static void tryAddPeer(const char* label, uint8_t* mac) {
  if (macAllZero(mac)) {
    Serial.printf("[WROOM L2] Set %s MAC in sketch (ESP_MAC_FINDER on that board).\n", label);
    return;
  }
  ...
  if (e == ESP_OK) {
    Serial.printf("[WROOM L2] Peer OK: %s\n", label);
  } else if (e == ESP_ERR_ESPNOW_EXIST) {
    /* already added */
  } else {
    Serial.printf("[WROOM L2] add_peer %s err %d\n", label, (int)e);
  }
}
...
static void onRecv(const esp_now_recv_info_t* info, const uint8_t* data, int len) {
  ...
  if (len == (int)sizeof(ImuPacket) && macEq(src, LEG2_IMU_MAC)) {
    memcpy(&lastImu, data, sizeof(ImuPacket));
    haveImu = true;
  } else if (len == (int)sizeof(EmgPacket) && macEq(src, LEG2_EMG_MAC)) {
...
static void sendCmdToImu(uint8_t cmd) {
  if (macAllZero(LEG2_IMU_MAC)) {
    Serial.println("[WROOM L2] Cannot send cmd — LEG2_IMU_MAC not set.");
    return;
  }
  CmdPacket p = {cmd};
  esp_err_t e = esp_now_send(LEG2_IMU_MAC, (uint8_t*)&p, sizeof(p));
  Serial.printf("[WROOM L2] Cmd %u → IMU: %s\n", (unsigned)cmd, e == ESP_OK ? "queued" : "fail");
}
...
void setup() {
  ...
  if (esp_now_init() != ESP_OK) {
    Serial.println("[WROOM L2] esp_now_init FAIL");
    while (true) {
      delay(1000);
    }
  }
  ...
  Serial.println("[WROOM L2] Ready. Serial: 1/c = cal, 2/r = enc reset");
  lastSerial = millis();
}
